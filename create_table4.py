...
from sqlalchemy_interbase.base import ib_types
...
class Sampletable5(AutoIncrementBase):
    __tablename__ ='SAMPLETABLE5'
    id = Column(IBDECIMAL(),
                Sequence(f'{__tablename__}_id_gen'),
                primary_key=True)
    name = Column(IBVARCHAR(length=50, charset='WIN1252', collation='WIN1252'))
    value = Column(IBDECIMAL())


class Sample_table(AutoIncrementBase):
    ...
    name = Column(
        IBVARCHAR(length=50, charset='WIN1252',
                  collation='WIN1252'))
    value = Column(IBDECIMAL())
